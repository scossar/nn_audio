...
t_class *updown3_class = NULL;

// #define XTRA_SAMPS 1

typedef struct _updown3 {
  t_object x_obj;
...
static void initilize_buffers(t_updown3 *x) {
  x->x_full_buffer = getbytes(sizeof(t_float) * (x->x_full_buffer_size));
  if (!x->x_full_buffer) {
    pd_error(x, "updown3~: unable to assign memory to full buffer");
    return;
  }

  x->x_downsampled_buffer = getbytes(sizeof(t_float) * x->x_downsampled_buffer_size);
  if (!x->x_downsampled_buffer) {
    pd_error(x, "updown3~: unable to assign memory to downsample buffer");
    return;
  }
}

static t_int *updown3_perform(t_int *w) {
  t_updown3 *x = (t_updown3 *)(w[1]);
  ...
  while (n--) {
    ...
    if ((input_single_phase % x->x_downsample_factor) == 0) {
      downsample_buff[downsample_single_phase] = filtered_in;
...
static void updown3_dsp(t_updown3 *x, t_signal **sp) {
  x->x_sr = sp[0]->s_sr;
  dsp_add(updown3_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_length);
}


static void updown3_free(t_updown3 *x) {
  if (x->x_full_buffer) {
...
static void *updown3_new(void) {
  t_updown3 *x = (t_updown3 *)pd_new(updown3_class);
...
void updown3_tilde_setup(void) {
  updown3_class = class_new(gensym("updown3~"),
                           (t_newmethod)updown3_new,
                           (t_method)updown3_free,
                           sizeof(t_updown3),
                           CLASS_DEFAULT,
                           0);

  class_addmethod(updown3_class, (t_method)updown3_dsp,
                  gensym("dsp"), A_CANT, 0);
  CLASS_MAINSIGNALIN(updown3_class, t_updown3, x_f);
}
