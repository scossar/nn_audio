...
// confirm this works. fast_inv_sqrt produces significantly different results
...
static void update_parameters_adam(t_nnlfofl *x) {
  ...
  for (int l = 0; l < x->x_num_layers; l++) {
    ...
    for (; j < num_weights; j++) {
      ...
      t_float update = alpha * layer->l_v_dw[j] * fast_inv_sqrt(layer->l_s_dw[j] + epsilon);
    ...
    for (; k < n; k++) {
      ...
      t_float update = alpha * layer->l_v_db[k] * fast_inv_sqrt(layer->l_s_db[k] + epsilon);
